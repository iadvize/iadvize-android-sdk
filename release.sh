...
if [ -f "IAdvizeSDK.zip" ]; then
    ...
    if [ -d "com/iadvize/iadvize-sdk/${versionName}" ]; then
        ...
        echo -e "\033[1;42m => Updating demo build.gradle.kts. \033[0m"
        sed -i '' "s/(\"com.iadvize:iadvize-sdk:\(.*\)\")/(\"com.iadvize:iadvize-sdk:${versionName}\")/" demo/mobile/build.gradle.kts
